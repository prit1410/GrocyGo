...
class InventoryApi {
  static String get baseUrl => AppConfig.inventoryEndpoint;
  ...
  static Future<List<InventoryItem>> fetchInventory({String? token}) async {
    ...
    print('[InventoryApi] GET $baseUrl');
    print('[InventoryApi] Status: ${res.statusCode}');
    print('[InventoryApi] Body: ${res.body}');
    if (res.statusCode == 200) {
      final data = json.decode(res.body);
      print('[InventoryApi] Decoded data: $data');
      if (data is List) {
        return data.map((e) => InventoryItem.fromJson(e)).toList();
      }
      print('[InventoryApi] Unexpected data type: ${data.runtimeType}');
    }
